...
<form  oninput="range_control_value.value = range_control.valueAsNumber"> 
  <table width="887" height="199" border="0">
    ...
    <tr>
      <td><p>Teléfono: </p></td>
      <td><input type="tel" name="ctel" value= "<?php if (isset($_GET['error'])) {$vtel =  $teldesencriptar($_GET['etelefono']); echo $vtel;} elseif(isset($_GET['tele'])) {$vtel = $_GET['etelefono']; echo $vtel;} else { echo ""; }?>" pattern"^[0-9]\d{7,10}$" title="El numero de telefono debe ser de 7 o 10 digitos" required /></td>
    </tr>
    <tr>
      <td><p>Dirección:</p></td>
    ...
    <tr>
      ...
      <td><input type="email" name="cemail" value= "<?php if (isset($_GET['error'])) { $vcorreo = $_GET['ecorreo'];$vcorreo = $desencriptar($vcorreo); echo $vcorreo;} else { echo ""; }?>" pattern="^[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+@[a-zA-Z0-9-]+(?:\.[a-zA-Z0-9-]+)*$" title="La direccion de correo electronico debe contener un dominio (@ejemplo.com o @ejemplo.co)"  required /></td>
    </tr>
  </table>
...
if (isset($_GET['add']) && $_GET['ctipo'] != "0"){
	...
	if (strlen($vtelef) != 7 && (strlen($vtelef) != 10 || $vtelef[0] != '3')){
		...
		include_once ("mcript.php");
		$vcorreo = $encriptar($vcorreo);
		header("Location: anadirEntidad.php?enit=$vnit&enombre=$vnombre&ecorreo=$vcorreo&etelefono=$vtelefono&edir=$vdir&error=$mensaje&tele=");
		die();
	}
	include_once ("mcript.php");
	$vcorreo = $encriptar($vcorreo);
	...
	if (mysqli_query($con,$sql)){
		...
		include_once("date.php");
		$inserta = "INSERT INTO log VALUES ('$user_ses','$f','Entidad Añadida Fallido','Se trataron de ingresar los datos de la entidad $vnit',1);";
		mysqli_query($con,$inserta);
		mysqli_close($con);
