...
<table width="1114" height="274" border="0">
  <tr>
    ...
    <td width="325"><input type="text" name="cnumdoc" value="<?php echo $row['cc']?>" pattern="^((\d{8})|(\d{10})|(\d{11})|(\d{6}-\d{5}))?$" required="required" readonly="readonly"/></td>
  ...
  <tr>
    <td><p>Teléfono:</p></td>
    <td><input type="tel" value="<?php echo $vtel?>" pattern"^[0-9]\d{7,10}$" title="El numero de telefono debe ser de 7 o 10 digitos" name="ctelefono" /></td>
  </tr>
  <tr>
    <td><p>Rol:</p></td>
    <td><select name="crol">
...
<?php
if (isset($_GET['mod']) && $_GET['crol'] != "0" && $_GET['ctipodoc'] != "0") {
	$vcc = $_GET['cnumdoc'];
	...
	if (!empty($_GET['csnombre'])){
		$vnombre2 = $_GET['csnombre'];
		$inserta = "UPDATE usuario SET cc = $vcc, tipo_doc = '$vtipodoc', pnombre = '$vnombre1', snombre = '$vnombre2', papellido = '$vapellido1', sapellido = '$vapellido2', fecha_nam = '$vfechanam', edad = $vedad, correo_electronico = '$vcorreo', telefono = '$vtelefono',rol = '$vrol' WHERE cc = '$vcc';";
	} else {
		$vnombre2 = "";
		$inserta = "UPDATE usuario SET cc = $vcc, tipo_doc = '$vtipodoc', pnombre = '$vnombre1', snombre = NULL, papellido = '$vapellido1´', sapellido = '$vapellido2', fecha_nam = '$vfechanam', edad = $vedad, correo_electronico = '$vcorreo', telefono = '$vtelefono', rol = '$vrol' WHERE cc = '$vcc';";
	}
	$vtelef = strval($vtelefono);
	if (strlen($vtelef) != 7 & strlen($vtelef) != 10){
		$m = "El número telefonico debe contener 7 o 10 digitos ($vtelef)";
		header("Location: modificarUsuario1.php?id=$vcc&error=$m");
		die();
	}
	if (mysqli_query($con,$inserta)){
		include_once("date.php");
		$inserta = "INSERT INTO log VALUES ('$user_ses','$f','Usuario Modificado Exitoso','Se modificaron los datos del Empleado:$vcc',1);";
		...
		include_once("date.php");
		$inserta = "INSERT INTO log VALUES ('$user_ses','$f','Usuario Modificado Fallido','Trato de ingresar los datos del usuario: $vcc',1);";
		mysqli_query($con,$inserta);
		$m = "Error con los datos que se trataron de modificar";
		mysqli_close($con);
		header("Location: modificarUsuario1.php?id=$vcc$error=$m");
		die();
	}
