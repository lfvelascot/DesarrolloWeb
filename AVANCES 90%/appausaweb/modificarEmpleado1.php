...
<table width="1114" height="274" border="0">
  <tr>
    ...
    <td width="325"><input type="text" name="cnumdoc" value="<?php echo $row['cc']?>" pattern="^((\d{8})|(\d{10})|(\d{11})|(\d{6}-\d{5}))?$" readonly="readonly" required="required"/></td>
...
<p align="center">
  <button type="submit" name="mod">Modificar Empleado</button></p>
</form>
...
if (isset($_GET['mod'])) {
	...
	if (!empty($_GET['csnombre'])){
		...
		$inserta = "UPDATE usuario SET tipo_doc = '$vtipodoc', pnombre = '$vnombre1', snombre = '$vnombre2', papellido = '$vapellido1', sapellido = '$vapellido2', fecha_nam = '$vfechanam', edad = $vedad, correo_electronico = '$vcorreo', telefono = '$vtelefono' WHERE cc = '$vcc';";
	} else {
		$vnombre2 = "";
		$inserta = "UPDATE usuario SET tipo_doc = '$vtipodoc', pnombre = '$vnombre1', snombre = NULL, papellido = '$vapellido1´', sapellido = '$vapellido2', fecha_nam = '$vfechanam', edad = $vedad, correo_electronico = '$vcorreo', telefono = '$vtelefono' WHERE cc = '$vcc';";
	}
	...
	if (mysqli_query($con,$inserta)){
		...
		include_once("date.php");
		$inserta = "INSERT INTO $bd.log VALUES ('$user_ses','$f','Empleado Modificado Fallido','Conflicto con los datos ($vcc, $vtipodoc, $vnombre1, $vapellido1, $vfechanam, $vedad, $vcorreo, $vtelefono, $vrol)');";
		mysqli_query($con,$inserta);
		mysqli_close($con);
		$m = "Conflicto con los datos ingresados";
		header("Location: modificarEmpleado1.php?id=$vcc&error=$m");
		die();
	}
}
?>
