  ...
  <table width="900" height="550" border="0">
    ...
    <tr>
      ...
      <option selected value="0">Seleccionar</option>
          <?php 
          $sql="SELECT entidad.nit AS nit, entidad.nombre_entidad AS nombre FROM entidad,entidades_contratadas WHERE entidad.nit = entidades_contratadas.nit_entidad AND entidad.tipo =  'EPS' AND entidades_contratadas.nit_empresa = $empresa_ses AND entidad.e = 1;";
          $result=mysqli_query($con,$sql);
          while($mostrar=mysqli_fetch_array($result)){
           ?>
  ...
  $con=mysqli_connect($host,$usuario,$clave,$bd) or die("ERROR");
  mysqli_set_charset($con,"utf8");
  if (isset($_GET['add']) && $_GET['ccajacompensacion'] != "0"&& $_GET['cfondopensiones'] != "0"&& $_GET['carl'] != "0" && $_GET['ceps'] != "0") {
	  ...
	  if(mysqli_num_rows($result) != 0){
		...
		if ($vtipocontrato == "Definido" & !empty($_GET['cfechainin']) & !empty($_GET['cfechafin']) & ($rol == "EMPLEADO" | $rol == "TALENTO HUMANO") & ($d1 < $d2)){
			...
			if (mysqli_query($con,$inserta)){
				include_once("date.php");
          		$inserta = "INSERT INTO log VALUES ('$user_ses','$f','Contrato Añadido Exitoso','Inserto los datos del Contrato:$vnumcontrato de la empresa :$vnitempresa',1);";
				...
				include_once("date.php");
				$inserta = "INSERT INTO log VALUES ('$user_ses','$f','Contrato Añadido Fallido','Conflicto con los datos del contrato del contrato',1);";
				mysqli_query($con,$inserta);
				$sql = "SELECT * contrato WHERE num_contrato = '$vnumcontrato' AND empleado = '$vdocempleado' AND empresa = '$vnitempresa';";
				$resultado = mysqli_query($con, $consulta);
				if (mysqli_num_rows($resultado) != 0){
					$m = "Ya existe un contrato asociado con esta empresa, con el mismo numero de contrato y con el mismo empleado";
			...
			if (mysqli_query($con,$inserta)){
				...
				include_once("date.php");
          		$inserta = "INSERT INTO log VALUES ('$user_ses','$f','Contrato Añadido Fallido','Conflico con los datos del contrato',1);";
          		mysqli_query($con,$inserta);
				$sql = "SELECT * contrato WHERE num_contrato = '$vnumcontrato' AND empleado = '$vdocempleado' AND empresa = '$vnitempresa';";
				$resultado = mysqli_query($con, $consulta);
				if (mysqli_num_rows($resultado) != 0){
