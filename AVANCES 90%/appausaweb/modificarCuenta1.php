...
<table width="915" height="206" border="0">
  <tr>
    ...
    <td width="572"><input type="text" name="cdusuario" value="<?php if ($b | $load){echo $vusario; } else { echo "";}?>" pattern="^((\d{8})|(\d{10})|(\d{11})|(\d{6}-\d{5}))?$" readonly="readonly" required="required"/></td>
  </tr>
  <tr>
    <td><p>Correo Electronico:</p></td>
    <td><input type="email" name="cmail" value="<?php if ($b | $load){echo $vcorreo ;} else { echo "";}?>" pattern="^[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+@[a-zA-Z0-9-]+(?:\.[a-zA-Z0-9-]+)*$" required="required"/></td>
  </tr>
  <tr>
      <td><p>Perfil:</p></td>
...
<?php
if (isset($_GET['add']) && $_GET['crol'] != "0" && $_GET['cperfil'] != "0") {
	$vusario = $_GET['cdusuario'];
	$vcorreo = $_GET['cmail'];
	$perfil = $_GET['cperfil'];
	$vestado = $_GET['cestado'];
		if ((strpos($vcorreo, ".com") !== false | strpos($vcorreo, ".co") !== false)){
			...
			if(mysqli_num_rows($result) == 0){
				...
				$sql = "UPDATE cuenta SET correo_electronico = '$correoe', estado = '$vestado', perfil = '$perfil' WHERE  usuario = '$vusario';"; 
				if (mysqli_query($con,$sql)){
					include_once("date.php");
					$inserta = "INSERT INTO log VALUES ('$user_ses','$f','Cuenta Modificado Exitoso','Se modificaron los datos de la cuenta del usuario : $vusario',1);";
					...
					include_once("date.php");
					$inserta = "INSERT INTO log VALUES ('$user_ses','$f','Cuenta Modificado Fallido','Conflicto con la actualizacion de los datos de la cuenta del usuario : $usuario',1);";
					mysqli_query($con,$inserta);
					mysqli_close($con);
					$m = "";
					header("Location:  modificarCuenta1.php?id=$vusario&react=$m");
					die();
				}
			} else {
				include_once("date.php");
				...
				mysqli_query($con,$inserta);
				mysqli_close($con);
				$m = "El perfil seleccionado no esta asociado al rol del usuario";
				header("Location:  modificarCuenta1.php?id=$vusario&react=$m");
				die();
			}
		} else {
			include_once("date.php");
			$inserta = "INSERT INTO log VALUES ('$user_ses','$f','Cuenta Modificado Fallido','El correo electronico no esta permitido',1);";
			mysqli_query($con,$inserta);
			mysqli_close($con);
			$m = "El correo electronico no contiene un dominio permitido";
			header("Location:  modificarCuenta1.php?id=$vusario&react=$m");
			die();
		}
}
