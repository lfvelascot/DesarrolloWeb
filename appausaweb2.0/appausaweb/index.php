...
<p align="center">Correo Electronico: <input type="text" name="ccorreo" pattern="^[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+@[a-zA-Z0-9-]+(?:\.[a-zA-Z0-9-]+)*$" autofocus required />
</p>

<p align="center">Contraseña:<input type="password" name="ccontrasena" autofocus required />
</p>
		...
		if ($estado == "ACTIVA"){
			...
			if ($contrasena == $dato_encriptado2){
				...
				if ($fallidos == 3){
					...
					$consulta = "UPDATE $bd.cuenta SET estado = 'BLOQUEDA' WHERE correo_electronico = '$dato_encriptado1';";
					...
					$sql="INSERT INTO $bd.log VALUES ('$dato_encriptado1','$f','Ingreso Fallido','La cuenta Fue Bloqueda por Exceso de Ingresos Fallidos',1);";
					...
					$consulta = "UPDATE $bd.cuenta SET intentos_fallidos = $fallidos WHERE correo_electronico = '$dato_encriptado1';";

					mysqli_query($con,$consulta);

					$sql="INSERT INTO $bd.log VALUES ('$dato_encriptado1','$f','Ingreso Fallido','Ingreso Fallido a APPAUSA',1);";
					...
					header("Location: ./?correo=$correo&failed=$mensaje");
			...
			$sql="INSERT INTO $bd.log VALUES ('$dato_encriptado1','$f','Ingreso Fallido','La cuenta de este usuario se encuentra BLOQUEADA, pero trato de ingresar',1);";
...
<a href="contrasenaOlvidada.html"> Olvide mi contraseña</a>
</p>
</div>
