...
<table width="915" height="206" border="0">
  <tr>
    ...
    <td width="572"><input type="text" name="cdusuario" value="<?php if ($b){echo $vusario; } else { echo "";}?>" pattern="^((\d{8})|(\d{10})|(\d{11})|(\d{6}-\d{5}))?$" required/></td>
  </tr>
  <tr>
    <td><p>Correo Electronico:</p></td>
    <td><input type="email" name="cmail" value="<?php if ($b){echo $vcorreo ; } else { echo "";}?>" pattern="^[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+@[a-zA-Z0-9-]+(?:\.[a-zA-Z0-9-]+)*$" required="required"/></td>
  </tr>
  <tr>
    <td><p>Contraseña:</p></td>
    <td><input type="password" name="ccontrasena" required /></td>
  </tr>
  <tr>
    <td><p>Confirmar Contraseña:</p></td>
    <td><input type="password" name="ccontrasenac"  required /></td>
  </tr>
  <tr>
      <td><p>Perfil:</p></td>
...
<p align="center">
  <button type="submit" name="add">Añadir Cuenta</button></p>
</form>
<?php
if (isset($_GET['add'])) {
	$vusario = $_GET['cdusuario'];
	...
	$matches = null;
	if ($vcontr1 == $vcontr2 & strlen($vcontr1) >= 8 & strlen($vcontr1) <= 16 & preg_match('/[A-Za-z].*[0-9]|[0-9].*[A-Za-z]/', $vcontr1)){
		$matches = null;
		if (1 === preg_match('/^[A-z0-9\\._-]+@[A-z0-9][A-z0-9-]*(\\.[A-z0-9_-]+)*\\.([A-z]{2,6})$/', $vcorreo, $matches) & (strpos($vcorreo, ".com") !== false | strpos($vcorreo, ".co") !== false)){
			...
			if(mysqli_num_rows($result) == 0){
				...
				$sql = "INSERT INTO $bd.cuenta VALUES ('$correoe','$conte','$vestado','$perfil',$vusario,1);"; 
				if (mysqli_query($con,$sql)){
					include_once("date.php");
					$inserta = "INSERT INTO $bd.log VALUES ('$user_ses','$f','Cuenta Añadido Exitoso','Inserto los datos del Usuario:$vusario idenficado por el correo: $correoe',1);";
					...
					include_once("date.php");
					$inserta = "INSERT INTO $bd.log VALUES ('$user_ses','$f','Cuenta Añadido Fallido','Conflico con los datos de la Cuenta($correoe)',1);";
					mysqli_query($con,$inserta);
					mysqli_close($con);
		...
		include_once("date.php");
		$inserta = "INSERT INTO $bd.log VALUES ('$user_ses','$f','Cuenta Añadido Fallido','Las contraseñas no considieron',1);";
		mysqli_query($con,$inserta);
		mysqli_close($con);
