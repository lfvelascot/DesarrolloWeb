...
<table width="1114" height="274" border="0">
  <tr>
    ...
    <td width="325"><input type="text" value ="<?php if (isset($_GET['error'])) {echo $vcc;} else {echo "";}?>" name="cnumdoc" pattern="^((\d{8})|(\d{10})|(\d{11})|(\d{6}-\d{5}))?$" required/></td>
  ...
  <tr>
    ...
    <td><input type="email" value="<?php if (isset($_GET['error'])) {echo $vcorreo;} else {echo "";}?>" name="ccorreo" pattern="pattern="^[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+@[a-zA-Z0-9-]+(?:\.[a-zA-Z0-9-]+)*$" required /></td>
    <td><p>Teléfono:</p></td>
    <td><input type="tel" namevalue="<?php if (isset($_GET['error'])) {echo $vtelefono;} else {echo "";}?>" ="ctelefono" /></td>
  </tr>
</table>
