...
<table width="1114" height="274" border="0">
  <tr>
    ...
    <td width="325"><input type="text" name="cnumdoc" value ="<?php if (isset($_GET['error'])) {echo $vcc;} else {echo "";}?>" 	pattern="^((\d{8})|(\d{10})|(\d{11})|(\d{6}-\d{5}))?$"/></td>
  ...
  <tr>
    ...
    <td><input type="email" name="ccorreo" value="<?php if (isset($_GET['error'])) {echo $vcorreo;} else {echo "";}?>" pattern="^[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+@[a-zA-Z0-9-]+(?:\.[a-zA-Z0-9-]+)*$" required /></td>
  </tr>
  <tr>
    <td><p>Teléfono:</p></td>
    <td><input type="number" value="<?php if (isset($_GET['error'])) {echo $vtelefono;} else {echo "";}?>" name="ctelefono" pattern"^[1-9]\d{9}$"/></td>
  </tr>
  <tr>
    <td><p>Rol:</p></td>
    <td><select name="crol">
...
if (isset($_GET['add'])) {
	...
	if (!empty($_GET['csnombre'])){
		...
		$inserta = "INSERT INTO $bd.usuario (`cc`, `tipo_doc`, `pnombre`,`snombre`, `papellido`,`sapellido`, `fecha_nam`, `edad`, `correo_electronico`, `telefono`, `rol`,`e`) VALUES ('$vcc','$vtipodoc','$vnombre1', '$vnombre2','$vapellido1' ,'$vapellido2','$vfechanam',$vedad,'$vcorreo','$vtelefono','$vrol',1);";
	} else {
		$vnombre2 = "";
		$inserta = "INSERT INTO $bd.usuario (`cc`, `tipo_doc`, `pnombre`,`snombre`, `papellido`,`sapellido`, `fecha_nam`, `edad`, `correo_electronico`, `telefono`, `rol`,`e`) VALUES ('$vcc','$vtipodoc','$vnombre1', NULL,'$vapellido1' ,'$vapellido2','$vfechanam',$vedad,'$vcorreo','$vtelefono','$vrol',1);";
	}
	...
	if (mysqli_query($con,$inserta)){
		...
		$inserta = "INSERT INTO $bd.log VALUES ('$user_ses','$f','Usuario Añadido Exitoso','Inserto los datos del Usuario:$vcc',1);";
		...
		include_once("date.php");
		$inserta = "INSERT INTO $bd.log VALUES ('$user_ses','$f','Usuario Añadido Fallido','($vcc,$vtipodoc,$vnombre,$vapellido,$vfechanam,$vedad,$vcorreo,$vtelefono,$vrol,1)',1);";
		mysqli_query($con,$inserta);
		mysqli_close($con);
