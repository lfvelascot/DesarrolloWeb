  ...
  <table width="900" height="550" border="0">
    <tr>
      ...
      <td><input type="number" value="<?php echo $mostrar['num_contrato'];?>" name="cnumcontrato" readonly="readonly" autofocus required /></td>
    </tr>
    <tr>
      <td><p>NIT Empresa:</p></td>
      <td><input type="text" value="<?php echo $mostrar['empresa'];?>" name="cnitempresa" pattern="^[0-9]{0,14}+-[{0-9}]$" readonly="readonly" autofocus required /></td>
    </tr>
      <tr>
      <td><p>Documento Empleado:</p></td>
      <td><input type="text" value="<?php echo $mostrar['empleado'];?>" name="cdocumentoemp" pattern="^((\d{8})|(\d{10})|(\d{11})|(\d{6}-\d{5}))?$" readonly="readonly" autofocus required /></td>
    </tr>
    <tr>
  ...
  if (isset($_GET['add']) && $_GET['ccajacompensacion'] != "0"&& $_GET['cfondopensiones'] != "0"&& $_GET['carl'] != "0" && $_GET['ceps'] != "0" && $_GET['ctipocontrato'] != "0") {
	  ...
	  if(mysqli_num_rows($result) != 0){
		...
		if ($vtipocontrato == "Definido" & !empty($_GET['cfechainin']) & !empty($_GET['cfechafin']) & ($rol == "EMPLEADO" | $rol == "TALENTO HUMANO")){
			$inserta = "UPDATE contrato SET num_contrato = '$vnumcontrato', empleado = '$vdocempleado', empresa = '$vnitempresa', cargo = '$vcargo', fecha_inicio = '$vfechaini', fecha_fin = '$vfechafin', sueldo = '$vsueldo', funciones = '$vfunciones', arl = '$varl', caja_compensación = '$vcajacompensacion', fondo_pensiones = '$vfondopensiones', eps = '$veps', tipo_contrato = '$vtipocontrato' WHERE num_contrato = '$vnumcontrato' AND empresa = '$vnitempresa';";
			if (mysqli_query($con,$inserta)){
				include_once("date.php");
          		$inserta = "INSERT INTO log VALUES ('$user_ses','$f','Contrato Modificado Exitoso','Se Modificaron los datos del Contrato: $vnumcontrato de la Empresa : $vnitempresa',1);";
				...
				include_once("date.php");
				$inserta = "INSERT INTO log VALUES ('$user_ses','$f','Contrato Modificado Fallido','Conflico con los datos del contrato('$vnumcontrato','$vdocempleado','$vnitempresa','$vcargo','$vfechaini','$vfechafin','$vsueldo','$vfunciones','$varl','$vcajacompensacion','$vfondopensiones','$veps','$vtipocontrato')',1);";
				mysqli_query($con,$inserta);
				mysqli_close($con);
			...
			$inserta = "UPDATE contrato SET num_contrato = '$vnumcontrato', empleado = '$vdocempleado', empresa = '$vnitempresa', cargo = '$vcargo', fecha_inicio = '$vfechaini', fecha_fin = NULL, sueldo = '$vsueldo', funciones = '$vfunciones', arl = '$varl', caja_compensación = '$vcajacompensacion', fondo_pensiones = '$vfondopensiones', eps = '$veps', tipo_contrato = '$vtipocontrato' WHERE num_contrato = '$vnumcontrato' AND empresa = '$vnitempresa';";
			if (mysqli_query($con,$inserta)){
				include_once("date.php");
          		$inserta = "INSERT INTO log VALUES ('$user_ses','$f','Contrato Modificado Exitoso','Se Modificaron los datos del Contrato: $vnumcontrato de la Empresa : $vnitempresa',1);";
          		mysqli_query($con,$inserta);
			...
			include_once("date.php");
			$inserta = "INSERT INTO log VALUES ('$user_ses','$f','Contrato Añadido Fallido','Trato de ingresar el contrato de un Administrado : $vdocempleado',1);";
			mysqli_query($con,$inserta);
			mysqli_close($con);
			header("Location: modificarContrato.php");
			die();
