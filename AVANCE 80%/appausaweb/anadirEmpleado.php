...
<table width="1114" height="274" border="0">
  ...
  <tr>
    ...
    <td><p>Teléfono:</p></td>
    <td><input type="tel" namevalue="<?php if (isset($_GET['error'])) {echo $vtelefono;} else {echo "";}?>" ="ctelefono" title="Aqui se indica el formato" pattern"^[0-9]\d{7,10}$" autofocus="autofocus" required="required"/></td>
  </tr>
</table>
...
if (isset($_GET['add']) && $_GET['ctipodoc'] != "0") {
	...
	if (strlen($vtelefono) != 7 | (strlen($vtelefono) != 10 && strlen($vtelefono))){
	...
	if (mysqli_query($con,$inserta)){
		...
		include_once("date.php");
		$inserta = "INSERT INTO log VALUES ('$user_ses','$f','Empleado Añadido Fallido','($vcc,$vtipodoc,$vnombre,$vapellido,$vfechanam,$vedad,$vcorreo,$vtelefono,1)',1);";
		mysqli_query($con,$inserta);
