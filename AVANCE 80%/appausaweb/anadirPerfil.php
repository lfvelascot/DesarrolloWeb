...
<?php
if (isset($_GET['add']) && $_GET['crol'] != "0") {
	$nombre = $_GET['cnombre'];
	$descrip = $_GET['cdescrip'];
	$vrol = $_GET['crol'];
	$inserta = "INSERT INTO perfil VALUES('$nombre','$descrip','$vrol',1);";
	if (mysqli_query($con,$inserta)){
		include_once("date.php");
		$inserta = "INSERT INTO log VALUES ('$user_ses','$f','Perfil Añadido Exitoso','Inserto los datos del Perfil:$nombre',1);";
		...
		include_once("date.php");
		$inserta = "INSERT INTO log VALUES ('$user_ses','$f','Perfil Añadido Fallido','Se trato de añadir los datos del perfil: $nombre',1);";
		mysqli_query($con,$inserta);
		mysqli_close($con);
