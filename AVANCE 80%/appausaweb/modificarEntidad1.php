...
<form  oninput="range_control_value.value = range_control.valueAsNumber"> 
  <table width="887" height="199" border="0">
      ...
      <td><input type="number" name="cnit" value= "<?php if (isset($_GET['nit'])) { echo $_GET['nit'];} else { echo ""; }?>" pattern="^[0-9]{0,14}+-[{0-9}]$" autofocus required /></td>
    </tr>
    <tr>
    ...
    <tr>
      <td><p>Teléfono: </p></td>
      <td><input type="tel" name="ctel" value= "<?php if (isset($_GET['nit'])) { echo $telefono;} else { echo ""; }?>" pattern"^[1-9]\d{9}$" required /></td>
    </tr>
    <tr>
      <td><p>Dirección:</p></td>
...
if (isset($_GET['add']) && $_GET['ctipo'] != "0"){
	...
	$sql = "UPDATE entidad SET nit = $vnit, nombre_empresa = '$vnombre', tipo = '$vtipo', telefono = '$vtelefono', direccion = '$vdir', correo = '$vcorreo' WHERE nit = '$cnit'";
	if (mysqli_query($con,$sql)){
		include_once("date.php");
		$inserta = "INSERT INTO log VALUES ('$user_ses','$f','Entidad Modificada Exitoso','Se ingresaron los datos de la Entidad: $vnit',1);";
		...
		include_once("date.php");
		$inserta = "INSERT INTO log VALUES ('$user_ses','$f','Entidad Modificada Fallido','Se trataron de ingresar los datos de la entidad $vnombre',1);";
		mysqli_query($con,$inserta);
		mysqli_close($con);
