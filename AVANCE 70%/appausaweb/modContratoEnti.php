...
?>
<div id="apDiv1">
<h1 align="center">Modificar Contrato (Empresa: <?php  echo $nit_empresa?> - Entidad: <?php echo $nit_entidad?>)</h1>
...
?>
  ...
  <form>
    ...
    <tr>
     ...
     <tr>
       <?php
		$sql = "SELECT * FROM entidades_contratadas WHERE nit_empresa = $nit_empresa AND nit_entidad = $nit_entidad;";
		$result=mysqli_query($con,$sql);
		if(mysqli_num_rows($result) != 0){
			while($mostrar=mysqli_fetch_array($result)){
?>
        <td><b>Fecha Inicio:</b></td>
        <td><input type="date" name="cfechainin" value = "<?php echo $mostrar['fecha_inicio'] ?>"autofocus required /></td>
      </tr>
      <tr>
        <td><b>Fecha Fin:</b></td>
        <td><input type="date" name="cfechafin" value = "<?php echo $mostrar['fecha_fin'] ?>" autofocus required /></td>
      </tr>
    </table>
 <p align="center"><button type="submit" name="add">Modificar Contrato</button></p>
 <?php
	}
}
 if (isset($_GET['add'])){
	 ...
	 if ($d1 < $d2){
		 $sql = "UPDATE entidades_contratadas SET fecha_inicio = '$vfechaini', fecha_fin = '$vfechafin' WHERE nit_empresa = $nit_empresa AND nit_entidad = $nit_entidad;;";
		...
		header("Location: modContratoEnti.php?emnit=$nit_empresa&ennit=$nit_entidad&error=$mensaje");
		die();
	 }
 }
 ?>
