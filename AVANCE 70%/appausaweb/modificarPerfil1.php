...
<table width="1114" height="274" border="0">
  ...
  <tr>
	...
	<?php } ?>
    <?php 
		$sql="SELECT * from rol WHERE nombre != 'SUPER' AND nombre != 'EMPLEADO';";
...
if (isset($_GET['add'])) {
	...
	$inserta = "UPDATE perfil SET nombre = '$nombre', descrpción = '$descrip',rol_asociado = '$vrol' WHERE nombre = '$vnombre';";
	if (mysqli_query($con,$inserta)){
		include_once("date.php");
		$vrolasos = $_SESSION['rol_cambiado'];
		$inserta = "INSERT INTO log VALUES ('$user_ses','$f','Perfil Modificado Exitoso','Modifico los datos del Perfil:$nombre    $vrol - $vrolasos',1);";
		mysqli_query($con,$inserta);
		if ($vrol != $vrolasos){
			$del = "DELETE FROM gestion_actividad WHERE perfil = '$nombre';";
		...
		include_once("date.php");
		$inserta = "INSERT INTO log VALUES ('$user_ses','$f','Perfil Modificado Fallido','Se trato de modificar los datos del perfil: $nombre',1);";
		mysqli_query($con,$inserta);
		mysqli_close($con);
