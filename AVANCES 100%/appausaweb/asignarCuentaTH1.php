...
<body>
  ...
  if (isset($_GET['num'])){
	  $vusario = $_GET['num'];
	  $upd = "UPDATE usuario SET rol = 'TALENTO HUMANO' WHERE cc = '$vusario';";
	  mysqli_query($con,$upd);
	  $m = "Ya puede crear la cuenta del Empleado de talento humano identificado con el documento $vusario";
  ...
  ?>
<div id="apDiv1">
<h1 align="center"> Crear Cuenta  Talento Humano </h1>
</div>
<div id="apDiv2">
...
<table width="915" height="206" border="0">
  <tr>
    ...
    <td width="572"><input type="text" name="cdusuario" value="<?php if (isset($_GET['react']) || isset($_GET['num'])){echo $vusario; } else { echo "";}?>" pattern="^((\d{8})|(\d{10})|(\d{11})|(\d{6}-\d{5}))?$" title="El numero de documento debe tener 5,6,8,10,11 digitos" readonly="readonly" required/></td>
  </tr>
  <tr>
    <td><p>Correo Electronico:</p></td>
    <td><input type="email" name="cmail" value="<?php if (isset($_GET['react']) || isset($_GET['num'])){echo $vcorreo ; } else { echo "";}?>" pattern="^[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+@[a-zA-Z0-9-]+(?:\.[a-zA-Z0-9-]+)*$" title="El correo eletronico debe contener un arroba para comprobar la existencia de un dominio" required="required"/></td>
  </tr>
  <tr>
  ...
  <tr>
      ...
      <?php 
          $sql="SELECT * FROM perfil WHERE rol_asociado != 'SUPER' AND rol_asociado != 'ADMINISTRADOR';";
				...
				if(mysqli_num_rows($result) != 0){
					...
					if(mysqli_num_rows($result) != 0){
						...
						if (mysqli_query($con,$sql)){
							...
							$inserta = "INSERT INTO log VALUES ('$user_ses','$f','Cuenta Añadido Exitoso','Se creo la cuenta para el empleado de Talento Humano identificado con el doc. $vusario',1);";
							...
							include_once("date.php");
							$inserta = "INSERT INTO log VALUES ('$user_ses','$f','Cuenta Añadido Fallido','Conflicto con los datos de la Cuenta del usuario($vusario)',1);";
							mysqli_query($con,$inserta);
							mysqli_close($con);
							...
							header("Location: asignarCuentaTH1.php?cdusuario=$vusario&cmail=$vcorreo&react=$m");
							die();
						}
					} else {
						include_once("date.php");
