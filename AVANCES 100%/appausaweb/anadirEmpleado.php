...
<table width="1114" height="274" border="0">
  ...
  <tr>
    ...
    <td><input type="email" value="<?php if (isset($_GET['error'])) {echo $vcorreo;} else {echo "";}?>" name="ccorreo" title="Aqui se indica el formato" pattern="^[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+@[a-zA-Z0-9-]+(?:\.[a-zA-Z0-9-]+)*$" autofocus="autofocus" required /></td>
    <td><p>Teléfono:</p></td>
    <td><input type="tel" value="<?php if (isset($_GET['error'])) {echo $vtelefono;} else {echo "";}?>" name="ctelefono" pattern"^[0-9]\d{7,10}$" title="El numero de telefono debe ser de 7 o 10 digitos" autofocus="autofocus" required="required"/></td>
  </tr>
</table>
<p align="center"><button type="submit" name="add">Registrar Empleado</button>
</form>
<?php
if (isset($_GET['add']) && $_GET['ctipodoc'] != "0") {
	$vcc = $_GET['cnumdoc'];
	...
	$tels = strval($vtelefono);
	if (strlen($tels) != 7 && (strlen($tels) != 10)){
	...
	if (!empty($_GET['csnombre'])){
		...
		$inserta = "INSERT INTO usuario (`cc`, `tipo_doc`, `pnombre`,`snombre`, `papellido`,`sapellido`, `fecha_nam`, `edad`, `correo_electronico`, `telefono`, `rol`,`e`) VALUES ('$vcc','$vtipodoc','$vnombre1', '$vnombre2','$vapellido1' ,'$vapellido2','$vfechanam',$vedad,'$vcorreo','$vtelefono','EMPLEADO',1);";
	} else {
		$vnombre2 = "";
		$inserta = "INSERT INTO usuario (`cc`, `tipo_doc`, `pnombre`,snombre, `papellido`,`sapellido`, `fecha_nam`, `edad`, `correo_electronico`, `telefono`, `rol`,`e`) VALUES ($vcc,'$vtipodoc','$vnombre1',NULL,'$vapellido1' ,'$vapellido2','$vfechanam',$vedad,'$vcorreo','$vtelefono','EMPLEADO',1);";
	}
	if (mysqli_query($con,$inserta)){
		...
		include_once("date.php");
		$inserta = "INSERT INTO log VALUES ('$user_ses','$f','Empleado Añadido Fallido','Trato de registrar los datos del empleado asociado al documento $vcc',1);";
		mysqli_query($con,$inserta);
		$sql = "SELECT * from usuario where cc = $vcc AND e = 1;";
		$resultado = mysqli_query($con, $sql);
		if (mysqli_num_rows($resultado) != 0){
			$m = "Ya existe un empleado/ usuario asociado con este numero de documento";
			echo "<script type='text/javascript'>alert('$m');</script>";
			mysqli_close($con);
			header("Location: buscarEmpleado.php?ccc=$vcc&buscar=");
			die();
		...
		mysqli_close($con);
		$mensaje = "Se genero algun problema con los datos ingresados";
		$vcorreo = $desencriptar($vcorreo);
		header("Location: anadirEmpleado.php?ecc=$vcc&enombre1=$vnombre1&enombre2=$vnombre2&eapellido1=$vapellido1&eapellido2=$vapellido2&efechanam=$vfechanam&eedad=$vedad&ecorreo=$vcorreo&etelefono=$vtelefono&error=$mensaje");
		die();
}
