...
<table width="1043" height="265" border="0">
  <tr>
    <td><p> NIT:</p></td>
    <td width="525"><input type="text" name="cnit" value= "<?php if (isset($_GET['cempresa'])) { echo $emp;} else { echo ""; }?>" pattern="^[0-9]{0,14}+-[{0-9}]$" readonly="readonly" autofocus required /></td>
  </tr>
  <tr>
  ...
  <tr>
    <td><p>Teléfono:</p></td>
    <td><input type="tel" name="ctel" value= "<?php if (isset($_GET['cempresa'])) { $vtel = $tel; $vtel =  $teldesencriptar($vtel); echo $vtel;} else { echo ""; }?>" pattern"^[0-9]\d{7,10}$" title="El numero de telefono debe ser de 7 o 10 digitos" required/></td>
  </tr>
  <tr>
...
if (isset($_GET['add']) && $_GET['ctipo'] != "0"){
	...
	if (mysqli_query($con,$sql)){
		...
		$inserta = "INSERT INTO log VALUES ('$user_ses','$f','Empresa Modificada Exitoso','Se modificaron los datos de la empresa: $nitemp',1);";
		...
		include_once("date.php");
		$inserta = "INSERT INTO log VALUES ('$user_ses','$f','Empresa Modificada Fallido','Se trataron de modificar los datos de la empresa: $nitemp',1);";
		mysqli_query($con,$inserta);
		mysqli_close($con);
