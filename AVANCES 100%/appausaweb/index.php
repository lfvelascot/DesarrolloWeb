...
<p align="center">Correo Electronico: <input type="text" name="ccorreo" value = "<?php if (isset($_GET['correo'])) {include ("mcript.php"); $c =  $desencriptar($_GET['correo']); echo $c;}?>" pattern="^[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+@[a-zA-Z0-9-]+(?:\.[a-zA-Z0-9-]+)*$" title="El correo debe contener un dominio, un arroba y finalizar con .com o .co" autofocus required /></p>
<p align="center">Contraseña:<input type="password" title="Ingrese una contraseña valida" name="ccontrasena" autofocus required /></p>
<button type="submit" title="De click aqui para validar" name="login">Ingresar</button>
</form>
<?php
	...
	if (mysqli_num_rows($resultado) != 0){
		...
		if ($estado == "ACTIVA"){
			if ($contrasena == $dato_encriptado2){
				...
				if ($_SESSION['rol']  == 'TALENTO HUMANO'){
					...
					if (mysqli_num_rows($resultado) != 0){
						...
						if (mysqli_num_rows($resultado) != 0){
							...
							include_once("date.php");		
							$sql="INSERT INTO log VALUES ('$dato_encriptado1','$f','Ingreso Fallido','No existe un contrato que lo asocie con ninguna empresa',1);";
							mysqli_query($con,$sql);
							session_destroy();
							$m = "Usted aun no cuenta con un contrato asociado a ninguna empresa (No se encontraron los horarios)";
							header("Location: ./?failed=$m");
							mysqli_close($con);
							exit;
						}
							...
							mysqli_query($con,$sql);
							session_destroy();
							$m = "Usted aun no cuenta con un contrato asociado a ninguna empresa (No enconto la empresa)";
							header("Location: ./?failed=$m");
							mysqli_close($con);
							exit;
					}
					...
					if ($hora >= $inicio && $hora <= $fin){
						...
						include_once("date.php");		
						$sql="INSERT INTO log VALUES ('$dato_encriptado1','$f','Ingreso Fallido','Problemas en el acceso  (Horario no permitido)',1);";
						mysqli_query($con,$sql);
						session_destroy();
				...
				if ($fallidos == 3){
					$consulta = "UPDATE cuenta SET estado = 'BLOQUEADA' WHERE correo_electronico = '$dato_encriptado1';";
					mysqli_query($con,$consulta);
					$consulta = "UPDATE cuenta SET intentos_fallidos = 0 WHERE correo_electronico = '$dato_encriptado1';";
					mysqli_query($con,$consulta);
